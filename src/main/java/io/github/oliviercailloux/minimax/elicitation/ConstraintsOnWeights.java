...
/**
 *
 * The weight of rank 1 is 1. If there is more than one rank, the weight of lowest rank is 0.
 *
 * Because of imprecision in linear programming optimization, we could end up with weights that are
 * non convex up to a very minor error. This is solved by adding a small epsilon
 * ({@link ConstraintsOnWeights#EPSILON}) to the constraints.
 *
 * <p>
 * The use of this added epsilon modifies the limits admitted by any “feasible” set of weights. This
 * modification may become non negligible when the rank is high. To understand this, consider this
 * example with 6 ranks and minimizing w2 (thus minimizing w3, …, w6). Setting w5 = 0, the
 * difference between w5 and w6 is zero. Because the difference between w4 and w5 must be at least
 * epsilon higher than the difference between w5 and w6, the difference between w4 and w5 must be at
 * least epsilon. Thus, w4 ≥ epsilon. By the same reasoning, the difference between w3 and w4 must
 * be at least two epsilon. Thus, w3 ≥ 3 epsilon. Finally, the difference between w2 and w3 must be
 * at least three epsilon. Thus, w2 ≥ 6 epsilon.
 * </p>
 * <p>
 * In general, the lower bound on w2 is computed as follows. w2 ≥ epsilon + 2 epsilon + 3 epsilon +
 * … + (m − 3) epsilon = (m² − 5m + 6)/2 epsilon.
 * </p>
 * <p>
 * For m = 20, we obtain w2 ≥ 153 epsilon.
 * </p>
 * <p>
 * This will be a problem in the following circumstance. Assume m = 20. It could be that w2 = 0 is a
 * feasible solution for a given problem in reality, but this program does not find any feasible
 * solution because w2 < 153 epsilon for all (really) feasible solutions. However, considering the
 * kind of constraints accepted by this class, it is difficult to find a problematic example of that
 * kind.
 * </p>
 * <p>
 * Another problem may happen when the coefficients (representing typically the number of voters)
 * are high. Consider the objective w1 − 10^4 w2, to be maximized. Setting w2 to zero would lead to
 * a value of 1d, but because w2 ≥ 153 epsilon (if m = 20), with epsilon = 10^−6, we obtain a
...
public class ConstraintsOnWeights {
  ...
  /**
   * @param m at least one: the number of ranks, or equivalently, the number of alternatives.
   */
  ...
  private ConstraintsOnWeights(int m) {
    checkArgument(m >= 1);
    builder = MP.builder();
    builder.addVariable(Variable.of("w", VariableDomain.REAL_DOMAIN, RangeOfDouble.closed(1d, 1d),
        ImmutableSet.of(1)));
    for (int rank = 2; rank < m; ++rank) {
      builder.addVariable(Variable.of("w", VariableDomain.REAL_DOMAIN, RangeOfDouble.ZERO_ONE_RANGE,
          ImmutableSet.of(rank)));
    }
    if (m >= 2) {
      builder.addVariable(Variable.of("w", VariableDomain.REAL_DOMAIN, RangeOfDouble.closed(0d, 0d),
          ImmutableSet.of(m)));
    }
   ...
   * Copy constructor.
   *
   * @param mp should come from another COW instance (to guarantee that the structure conforms to
   *        expectations).
   * @param convexityConstraintSet should come from the same instance (to guarantee coherence).
   */
