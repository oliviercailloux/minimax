...
public class Run {
  ...
  private static ImmutableList<Integer> getQuestionTimesMs(List<Long> startTimes, long endTime) {
    ...
    verify(times.stream().mapToInt(Integer::intValue).sum() == Math
        .toIntExact(endTime - startTimes.get(0)));
    return times;
  }

  @JsonbCreator
  public static Run of(@JsonbProperty("oracle") Oracle oracle,
      @JsonbProperty("questions") List<Question> questions,
      @JsonbProperty("timesMs") List<Integer> durationsMs) {
    return new Run(oracle, questions, durationsMs);
  }

  public static Run of(Oracle oracle, List<Long> startTimes, List<Question> questions,
      long endTime) {
  ...
  public int getNbQVoters() {
    return (int) questions.stream().filter((q) -> q.getType().equals(QuestionType.VOTER_QUESTION))
        .count();
  }
  ...
  public int getNbQCommittee() {
    return (int) questions.stream()
        .filter((q) -> q.getType().equals(QuestionType.COMMITTEE_QUESTION)).count();
  }
  ...
  public ImmutableList<Regrets> getMinimalMaxRegrets() {
    if (regrets == null) {
      ...
      final ImmutableList.Builder<Regrets> builder =
          ImmutableList.builderWithExpectedSize(questions.size() + 1);
  ...
  private double computeLoss(int i) {
    ...
    final ImmutableList<Double> allLosses =
        chosen.stream().map(x -> oracle.getBestScore() - oracle.getScore(x))
  ...
  /**
   * The loss is the “effective regret”: defining x as an alternative we recommend, the loss when
   * choosing x is the score of x minus the score of y, considering the effective preference data
   * (the oracle), and not merely what we know about it.
   *
   * We can in principle recommend more than one alternative (in case of tied MMR). The loss is the
   * average of the losses when choosing each of the recommended alternatives.
   *
   * @return a list of size k + 1.
   */
  @JsonbTransient
  public ImmutableList<Double> getLosses() {
    ...
    losses = IntStream.rangeClosed(0, getK()).mapToObj(this::computeLoss)
        .collect(ImmutableList.toImmutableList());
    return losses;
  }
  ...
  public boolean equals(Object o2) {
    ...
    final Run r2 = (Run) o2;
    return oracle.equals(r2.oracle) && questions.equals(r2.questions)
        && durationsMs.equals(r2.durationsMs);
  }
