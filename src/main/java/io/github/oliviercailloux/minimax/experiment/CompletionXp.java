...
public class CompletionXp {
  ...
  public void elicitRandomly() {
    ...
    final VoterPartialPreference knowledge =
        VoterPartialPreference.about(oracle.getVoter(), alternatives);
    ...
    for (int i = 0; i <= 100; ++i) {
      ...
      for (Alternative a1 : alternatives) {
        ...
        if (question != null) {
          break;
          // final Set<Alternative> adjacentNodes =
          // knowledge.asTransitiveGraph().adjacentNodes(alternative);
          // final int questionsTaken = (int) adjacentNodes.stream()
          // .filter(a -> Comparator.<Alternative>naturalOrder().compare(alternative, a) <
          // 0).count();
