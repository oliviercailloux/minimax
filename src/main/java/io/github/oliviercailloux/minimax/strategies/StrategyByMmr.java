 ...
 * </p>
 * <p>
 * It is possible to constrain this strategy for asking the first q questions only to voters (or
 * only to committee), then the next q' questions only to committee (or to voters), and so on. BUT
 * this constraint will be lifted if it is impossible to satisfy: for example, if the profile is
 * entirely known and the constraint mandates to ask the next question to voters, this strategy will
 * anyway ask a question to the committee.
 * </p>
 **/
public class StrategyByMmr implements Strategy {
  ...
  private static class QuestioningConstraints {
    ...
    private QuestioningConstraints(List<QuestioningConstraint> constraints) {
      if (!constraints.isEmpty()) {
        checkArgument(constraints.stream().limit(constraints.size() - 1)
            .map(QuestioningConstraint::getNumber).noneMatch(n -> n == Integer.MAX_VALUE));
      }
    ...
    public boolean hasCurrentConstraint() {
      if (!constraints.isEmpty()
          && constraints.get(constraints.size() - 1).getNumber() == Integer.MAX_VALUE) {
        return true;
      }
      final int nbConstraints = constraints.stream().mapToInt(QuestioningConstraint::getNumber)
    ...
    public boolean mayAskCommittee() {
      return hasCurrentConstraint() ? getCurrentConstraint().equals(QuestionType.COMMITTEE_QUESTION)
          : true;
    }

    public boolean mayAskVoters() {
      return hasCurrentConstraint() ? getCurrentConstraint().equals(QuestionType.VOTER_QUESTION)
          : true;
    }
  }
  ...
  public static StrategyByMmr limited(Comparator<MmrLottery> comparator,
      List<QuestioningConstraint> constraints) {
  ...
  public Question nextQuestion() {
    ...
    final boolean allowVoters =
        (constraints.mayAskVoters() || m == 2) && !helper.getKnowledge().isProfileComplete();

    LOGGER.debug("Next question, allowing committee? {}; allowing voters? {}.", allowCommittee,
        allowVoters);

    if (helper.getKnowledge().isProfileComplete() && !allowCommittee) {
      LOGGER.debug("Asking a dummy question to voter.");
      final UnmodifiableIterator<Alternative> iterator =
          helper.getKnowledge().getAlternatives().iterator();
      Question q = Question.toVoter(helper.getKnowledge().getVoters().iterator().next(),
    ...
    if (limited) {
      if (allowVoters) {
        final ImmutableSetMultimap<Alternative, PairwiseMaxRegret> mmrs =
            helper.getMinimalMaxRegrets().asMultimap();

        final Alternative xStar =
            helper.drawFromStrictlyIncreasing(mmrs.keySet().asList(), Comparator.naturalOrder());
        final ImmutableSet<PairwiseMaxRegret> pmrs = mmrs.get(xStar);
        final PairwiseMaxRegret pmr =
            helper.drawFromStrictlyIncreasing(pmrs.asList(), PairwiseMaxRegret.BY_ALTERNATIVES);
      ...
      if (allowCommittee) {
        IntStream.rangeClosed(1, m - 2).boxed().forEach(
            i -> questionsBuilder.add(Question.toCommittee(helper.getQuestionAboutHalfRange(i))));
      }
    } else {
      if (allowVoters) {
        helper.getPossibleVoterQuestions().stream()
            .forEach(q -> questionsBuilder.add(Question.toVoter(q)));
      }
      if (allowCommittee) {
    ...
    questions = questionsBuilder.build().stream()
        .collect(ImmutableMap.toImmutableMap(q -> q, this::toLottery));
    verify(!questions.isEmpty());

    final Comparator<Question> questionsComparator =
        Comparator.comparing(q -> adjustLottery(q, questions.get(q)), lotteryComparator);
    final ImmutableSet<Question> bestQuestions =
        StrategyHelper.getMinimalElements(questions.keySet(), questionsComparator);
  ...
  private QuestionVoter getLimitedQuestion(Alternative xStar, Alternative yBar, Voter voter) {
    final ImmutableGraph<Alternative> graph =
        helper.getKnowledge().getPartialPreference(voter).asTransitiveGraph();
    ...
    if (!graph.adjacentNodes(xStar).contains(yBar)) {
      ...
      final Comparator<EndpointPair<Alternative>> comparingPair =
          Comparator.comparing(EndpointPair::nodeU);
      final Comparator<EndpointPair<Alternative>> c2 =
          comparingPair.thenComparing(EndpointPair::nodeV);
  ...
  private Optional<QuestionVoter> getQuestionAboutIncomparableTo(Voter voter,
      Graph<Alternative> graph, Alternative a) {
    final ImmutableSet<Alternative> incomparables =
        StrategyHelper.getIncomparables(graph, a).collect(ImmutableSet.toImmutableSet());
