...
public class StrategyElitist implements Strategy {
  ...
  public Question nextQuestion() {
    ...
    final ImmutableCollection<VoterPartialPreference> prefs =
        helper.getKnowledge().getProfile().values();
    for (VoterPartialPreference pref : prefs) {
      ...
      final Alternative topAlternative =
          alternatives.stream().max(Comparator.comparing(a -> graph.successors(a).size())).get();
    ...
    final Comparator<Question> questionsComparator =
        Comparator.comparing(questions::get, MmrLottery.MAX_COMPARATOR);
