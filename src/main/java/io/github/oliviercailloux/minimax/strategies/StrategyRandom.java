...
public class StrategyRandom implements Strategy {
  ...
  public Question nextQuestion() {
    ...
    if (askVoters) {
      final Voter voter =
          helper.drawFromStrictlyIncreasing(questionableVoters.asList(), Voter.BY_ID);
      final ImmutableGraph<Alternative> graph =
          helper.getKnowledge().getPartialPreference(voter).asTransitiveGraph();
      final ImmutableSetMultimap<Alternative, Alternative> incomparables =
          graph.nodes().stream().collect(ImmutableSetMultimap.flatteningToImmutableSetMultimap(
              a -> a, a -> StrategyHelper.getIncomparables(graph, a)));
      final Alternative a =
          helper.sortAndDraw(incomparables.keySet().asList(), Comparator.naturalOrder());
      final Alternative b =
          helper.sortAndDraw(incomparables.get(a).asList(), Comparator.naturalOrder());
