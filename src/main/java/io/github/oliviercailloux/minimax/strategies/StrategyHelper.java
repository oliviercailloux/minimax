...
public class StrategyHelper {
  ...
  public static ImmutableSet<EndpointPair<Alternative>>
      getIncomparablePairs(Graph<Alternative> graph) {
  ...
  public static <K, V extends Comparable<V>> ImmutableSet<K>
      getMinimalElements(Map<K, ? extends V> elements) {
    return getMinimalElements(elements, Comparator.naturalOrder());
  }

  public static <K, V> ImmutableSet<K> getMinimalElements(Map<K, ? extends V> elements,
    ...
    final ImmutableSet<K> minKeys =
        minValueOpt.isEmpty() ? ImmutableSet.of() : keysByValue.get(minValueOpt.get());
  ...
  public static <K> ImmutableSet<K> getMinimalElements(Set<K> elements, Comparator<K> comparator) {
    final ImmutableSet<K> sorted =
        elements.stream().sorted(comparator).collect(ImmutableSet.toImmutableSet());
    if (sorted.isEmpty()) {
      return sorted;
    }
    final K min = sorted.iterator().next();
    return sorted.stream().filter(e -> comparator.compare(e, min) == 0)
  ...
  private ImmutableSet<Integer> getRanksWithLambdaRangesWiderThan(double threshold) {
    return IntStream.rangeClosed(1, getM() - 2).boxed()
        .filter(r -> getWidthOfLambdaRangeAtRank(r) > threshold)
        .collect(ImmutableSet.toImmutableSet());
  }
  ...
  public ImmutableSet<QuestionCommittee> getQuestionsAboutLambdaRangesWiderThan(double threshold) {
    return getRanksWithLambdaRangesWiderThan(threshold).stream()
        .map(this::getQuestionAboutHalfRange).collect(ImmutableSet.toImmutableSet());
  }

  /**
   * @return an empty set iff m == 2.
   */
  public ImmutableSet<QuestionCommittee>
      getQuestionsAboutLambdaRangesWiderThanOrAll(double threshold) {
    final ImmutableSet<QuestionCommittee> questionsThreshold =
        getQuestionsAboutLambdaRangesWiderThan(threshold);
    ...
    if (getM() > 2) {
      /**
       * This fails iff the knowledge includes exact points for all lambdas (equivalently, if all
       * lambda ranges are empty). Considering the way our experiments are currently designed, we
       * assume this has zero probability.
       */
  ...
  public QuestionCommittee getQuestionAboutHalfRange(int rank) {
    final Range<Aprational> lambdaRange = getKnowledge().getLambdaRange(rank);
    final Aprational avg = AprationalMath
        .sum(lambdaRange.lowerEndpoint(), lambdaRange.upperEndpoint()).divide(new Apint(2));
  ...
  public PSRWeights getMinTauW(PairwiseMaxRegret pmr) {
    final ImmutableSortedMultiset<Integer> multiSetOfRanksOfX =
        ImmutableSortedMultiset.copyOf(pmr.getRanksOfX().values());
    final ImmutableSortedMultiset<Integer> multiSetOfRanksOfY =
        ImmutableSortedMultiset.copyOf(pmr.getRanksOfY().values());

    final RegretComputer regretComputer = getRegretComputer();

    final SumTerms sumTerms =
        regretComputer.getTermScoreYMinusScoreX(multiSetOfRanksOfY, multiSetOfRanksOfX);
